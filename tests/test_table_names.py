...
def test_load_table_names_from_cache(tmp_path):
    ...
    with patch("survivorpy.table_names._CACHE_TABLE_NAME_PATH", str(cache_path)):
...
def test_load_table_names_refresh_downloads_from_s3(tmp_path):
    ...
    with patch("survivorpy.table_names._CACHE_TABLE_NAME_PATH", str(tmp_path / "table_names.json")), \
